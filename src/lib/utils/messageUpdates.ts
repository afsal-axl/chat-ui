...
export async function uploadText(text: string): Promise<string> {
	...
	const params: UploadTextParams = {
		client_id: env.ST2_API,
		client_secret: env.CLIENT_SECRETS,
		tenant_id: env.CLIENT_IDS,
		text: `${text?.trim()}`,
		usr_id: env.USR_IDS,
	};
	...
	try {
		const response = await axios.post("https://axlstack.accelnomics.com/api/v1/executions", finalObj, {
			headers: headers
		},
		);
		return response.data;
	} catch (error: any) {
		console.error('Error uploading text:', error.response?.data || error.message);
		console.error('Status code:', error.response?.status);
		console.error('Headers:', error.response?.headers);
		return 'Failed to upload text';
	}
}
