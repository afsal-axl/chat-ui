<script lang="ts">
	...
	export let name;
	export let propToggle;
	const toggle = () => ($propToggle.useSearch = !$propToggle.useSearch);
</script>
...
<div
	...
	on:click={toggle}
	on:keydown={toggle}
	aria-checked={$propToggle.useSearch}
	aria-label="web search toggle"
	role="switch"
	tabindex="0"
>
	<Switch name="useSearch" bind:checked={$propToggle.useSearch} on:click on:keydown />
	<div class="whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{name}</div>
