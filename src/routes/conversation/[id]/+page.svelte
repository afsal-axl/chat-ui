<script lang="ts">
	...
	import { saveOutput } from "$lib/stores/saveOutput.js";
	import { uploadText } from "$lib/utils/messageUpdates";
	...
	async function writeMessage({
		...
		try {
			...
			console.log(messageToWriteTo,"find me")
			if (!messageToWriteTo) {
			...
			for await (const update of messageUpdatesIterator) {
				...
				if (update.type === "finalAnswer") {
						if($saveOutput.useSearch){
							uploadText(update.text)
						}
					loading = false;
					pending = false;
					break;
			...
			console.log(messageToWriteTo.updates)
		} catch (err) {
